...
- **Riepilogo stagioni (StagioniPage)** 🏁: le stagioni con partite app sono calcolate live da `computeSeasonRecap` (`utils/seasonRecap.js`) su TUTTE le partite Firestore della stagione (doc `isHistorical` della parte pre-app + partite live) e **sostituiscono** l'entry statica con lo stesso id in `HISTORICAL_SEASONS` — mai sommarle (doppio conteggio). Le righe giocatore passano da `aggregatePlayerMatchStats` (source of truth, con proration assist storici): non re-implementare l'aggregazione. I roster senza id (storici mai collegati) diventano righe residue con `gol: null` (ignoto ≠ 0). Export CSV per stagione via `buildSeasonCSV` (stessa forma per stagioni statiche e live).
