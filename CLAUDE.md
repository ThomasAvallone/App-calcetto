...
- **Partite storiche nei periodi** ⚖️: i doc `isHistorical` si escludono **solo** nell'all-time (dove al loro posto si sommano le `p.historicalStats`, per garantire `P == p.stats.matches`). Per `season`/`30d` quella compensazione non esiste, quindi vanno **contate** (`computeStandings`): hanno punteggio e roster con id. Tutte le altre viste per periodo (tab stats di StatsPage via `computeStatsFromMatches`, `PlayersPage.playerSeasonStats`, `DashboardPage.myStats`, Annali) filtrano solo per data e le includono: una nuova vista per periodo deve fare lo stesso, o i numeri della stessa stagione divergono tra pagine.
