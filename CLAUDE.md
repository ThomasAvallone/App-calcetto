...
│   ├── seasonRecap.js / seasonRecap.test.js  # riepilogo stagione live (StagioniPage + export CSV stagione)
...
| `leaderboards.js` | `computeStandings`, `computeDuoStats`, `computeH2HStats`, `computeSquadreStats`, `rankGoalkeepers`, `getSeasonStartMs`, `filterByPeriod`/`periodCutoffMs` (finestra per periodo, condivisa) |
...
| `eventMinute.js` | `deriveEventMinute` (fallback live + clamp monotonicità su `lastEventMinute`) + `fixZeroMinutes` (fix retroattivo; guard: non tocca partite con timing già noto) + `hasKnownTiming`/`maxEventMinute` |
| `seasonRecap.js` | `computeSeasonRecap` (riepilogo stagione da Firestore: storiche+app, stessa forma di HISTORICAL_SEASONS), `seasonIdOf`/`seasonBounds`/`listAppSeasonIds` |
...
- **Dati storici** (`historicalData.js`): invarianti garantiti da test → `players.length === totalPlayers` e `vinte+nulle+perse === presenze` per ogni giocatore. La classifica all-time ci si appoggia. **Stato noto del dataset** (pinnato in `historicalData.test.js`): dal 2020-21 in poi `HISTORICAL_MATCHES` combacia esattamente coi totali dichiarati; le due stagioni più vecchie sono incomplete (mancano 1 partita nel 2018-19 e 2 nel 2019-20) e 2 partite hanno l'anno sbagliato (ago 2018 invece di 2019), finendo fuori da ogni stagione. Per questo StagioniPage mostra la entry dichiarata quando è più completa del calcolo dal vivo.
...
- **Periodi (`all`/`season`/`30d`)** ⚖️: la finestra temporale si calcola SOLO con `filterByPeriod(matches, period, now)` (`utils/leaderboards.js`) — usata da StatsPage, PlayersPage, DashboardPage e ReportAITab. Non re-implementare il filtro inline: era esattamente così che i numeri della stessa stagione divergevano tra pagine. Regola incapsulata: si filtra **solo per data**, i doc `isHistorical` **contano**. L'unica eccezione è l'all-time di `computeStandings`, che li esclude perché al loro posto somma `p.historicalStats` (garantisce `P == p.stats.matches`); lì GF/GS restano app-only.
...
- **Minuto eventi** ⏱️ (`utils/eventMinute.js`): il minuto live viene dal cronometro manuale; se mai avviato, fallback sul tempo reale da `match.date` accettato solo se plausibile (0..150'). ⚠️ `match.date` NON è il calcio d'inizio reale (è la data scelta al setup). `deriveEventMinute` applica un **clamp di monotonicità** su `lastEventMinute` (cronaca mai a ritroso: report/replay/dettaglio ordinano per minuto e ci calcolano i parziali). `minute: 0` in una partita senza timing noto significa "sconosciuto", NON "primo minuto": i badge basati sul minuto (blitz, uno_due, early_bird) usano `hasKnownTiming(m.events)` come guard — mantenerla per ogni nuovo badge a base minuto. `fixZeroMinutes` (card admin in MatchDetailPage, solo `finished`) non tocca le partite con timing già noto (gli 0 residui sono gol legittimi del 1° minuto) e non assegna mai minuti > 150 (timestamp anomali).
- **Riepilogo stagioni (StagioniPage)** 🏁: le stagioni con partite app sono calcolate live da `computeSeasonRecap` (`utils/seasonRecap.js`) su TUTTE le partite Firestore della stagione (doc `isHistorical` della parte pre-app + partite live) e **sostituiscono** l'entry statica con lo stesso id in `HISTORICAL_SEASONS` — mai sommarle (doppio conteggio) — ma solo se la live copre almeno le stesse partite (se lo storico non è importato la statica è più completa e vince). Le righe giocatore passano da `aggregatePlayerMatchStats` (source of truth, con proration assist storici): non re-implementare l'aggregazione. Casi da preservare: i roster **senza id** (storici mai collegati) diventano righe residue con `gol: null` (ignoto ≠ 0) e vengono **riconciliati per nome/`historicalNames`** sulla riga del giocatore collegato (altrimenti la stessa persona produce due righe); i roster con id **non più in `players`** (giocatori eliminati) sono aggregati come player sintetici, altrimenti sparirebbero. I record passano tutti da `topEntry`, che unisce i **pari merito** come nei dati storici (`'Dani / Salmi'`, `'ottobre e aprile'`); le chiavi dei marcatori sono `scorerId`, mai `scorerName` (gli eventi storici possono non averlo e un rename spezzerebbe il conteggio). `unattributedGoals` = gol del punteggio che nessuna riga rivendica: è di quanto la colonna G non torna, mostrato in pagina e nel CSV. `withSeasonProgressFlag` riallinea il flag `inCorso` **hardcoded** delle entry statiche alla data corrente (e StagioniPage rinfresca `now` su `visibilitychange`), altrimenti la 2025/26 resterebbe "In Corso" per sempre: la stagione si chiude da sola il 1° settembre. Export CSV per stagione via `buildSeasonCSV` (stessa forma per stagioni statiche e live).
- **loadMatch refresh** (`matchStore.loadMatch`): se la partita richiesta è GIÀ in store (stesso id, match presente) il load è un **refresh subscription-first**: ri-sottoscrive `subscribeToMatch`/`subscribeToMatchState` e fa un `getMatch` fire-and-forget solo per riattivare il transport dopo un background — **senza** applicare il risultato del getDoc a `match`/`timerState` (eviterebbe di perdere optimistic update o resettare il cronometro su read fallita) e senza mai mostrare errori. Il cold load (match assente o id diverso) mantiene `loadError`/toast. `unloadMatch` **invalida `_loadToken`**: un load in volo che completa dopo l'unmount non deve risuscitare lo stato né creare subscription orfane. MatchPage richiama `loadMatch(id)` su `visibilitychange` (partita `active`, no pending writes) per recuperare i socket sospesi dal background su mobile.
